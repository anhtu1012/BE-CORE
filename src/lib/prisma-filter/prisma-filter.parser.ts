...
import { ArrayFilter, IntFilter, StringFilter } from './prisma-filter.type';
...
export class FilterParser<TDto, TWhereInput> {
  ...
  private generateWhere({
    filter = [],
    orFilter = [],
  }: IFilterOr<TDto>): TWhereInput & { OR?: TWhereInput[] } {
    ...
    for (const filterEntry of orFilter) {
      ...
      const dbFieldNameParts = dbFieldName.split('.');
      const orCondition: any = Object.create(null);
  ...
  private generateOrder(
    ...
    const generatedOrder: Array<{ [p in keyof TWhereInput]?: FilterOrder }> =
      [];
